...
* `sort-key`: A plain string used as the sort key when `sort: "symb"` is set in `#print-nomenclature`. Useful for subscripted symbols (e.g., `sort-key: "m1"` for `$m_1$`). Defaults to `none`.
